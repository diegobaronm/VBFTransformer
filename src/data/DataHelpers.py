...
from src.data.DataFormat import particle_feature_dict
...
def calculate_KDE_sampler(y_train, KDE_width, Min_Dens_cap):
    ...
    y_kde_train = resample(y_train_np, n_samples=20_000, random_state=42)
    kde = KernelDensity(kernel='gaussian', bandwidth=KDE_width).fit(y_kde_train.reshape(-1, 1))

    # Estimate density in parallel
