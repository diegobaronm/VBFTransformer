...
class SimpleDNN(nn.Module):
    def __init__(self, n_inputs, hidden_layers, dropout_prob=0.1, output_activation='linear'):
    ...
    def forward(self, x):
        ...
        y = self.layers[-1](x)
        y = self.apply_output_activation(y)
...
# Transformer model which takes two inputs to be passed to different embedders and then to be concatenated before self-attention
class ConcatenationTransformer(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        # configs
        inter_dim   = config['event_dim'] 
        part_dim    = config['input_dim']    
        nhead       = config['num_heads']
        num_layers  = config['num_layers']
        dnn_layers  = config['input_embedder_NN_layers']
